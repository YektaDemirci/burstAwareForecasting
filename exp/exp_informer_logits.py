...
class Exp_Informer(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x,batch_y,batch_x_mark,batch_y_mark, batch_x_peak, batch_y_peak, batch_x_idx, batch_y_idx) in enumerate(train_loader):
                ...
                y_peak = batch_y_idx[:, -self.args.pred_len:].float().to(self.device)
                loss_forecast = criterion(pred, true)
                alpha = 10
                loss_peak = F.binary_cross_entropy_with_logits(peak_logits.squeeze(-1), y_peak)

                loss = loss_forecast + alpha * loss_peak

                                # # Mixed loss
                # y_s = batch_y_idx[:, -self.args.pred_len:]
                # mask = y_s.any(dim=1)
                ...
                # delta = 300
                # # Ones are very sparse, zeros will be certainly there
                # # If the pred length goes too high, we can adjust this
                # if ones_l:
                #     loss_zeros = criterion(pred[~mask], true[~mask])
                #     loss_ones = criterion(pred[mask], true[mask]) * delta
                #     loss = ( (ones_l * loss_ones) + (zeros_l * loss_zeros) ) / (ones_l+zeros_l)
                # else:
                ...
                # # Self code here:
                # true_cpu = true.detach().cpu().numpy()[0][0][0]
                # pred_cpu = pred.detach().cpu().numpy()[0][0][0]
                # loss_cpu = loss.detach().cpu().numpy()
                # print(abs(true_cpu-pred_cpu))
                # print(loss_cpu)
        ...
        self.model.load_state_dict(torch.load(best_model_path))
            ...
            # adjust_learning_rate(model_optim, epoch+1, self.args)
            # train_loss_path = path+'/'+'train_loss_batch.csv'
            # np.savetxt(train_loss_path, loss_batch, delimiter=",")
            # train_loss_path = path+'/'+'train_loss_epoch.csv'
            # np.savetxt(train_loss_path, loss_epoch, delimiter=",")
            # best_model_path = f'{path}/bestcheckpoint.pth'
            # self.model.load_state_dict(torch.load(best_model_path))
    ...
    def test(self, setting):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark, batch_x_peak, batch_y_peak, batch_x_idx, batch_y_idx) in enumerate(test_loader):
            ...
            # pred_inversed = test_data.inverse_transform(pred)
            # pred_inverseds = pred_inversed.detach().cpu().numpy()
            # print(pred_inverseds)
    ...
    def predict(self, setting, load=False):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark, batch_x_peak, batch_y_peak, batch_x_idx, batch_y_idx) in enumerate(pred_loader):
            ...
            #     x = pred_data.inverse_transform(pred_data.data_x)
            #     x_inp = pred_data.inverse_transform(batch_x)
            #     y_inp = pred_data.inverse_transform(batch_y)
            #     print(x_inp[0][:10])
        ...
        # trues_inv_2d = pd.DataFrame(np.hstack((dates, trues_inv_2d)))
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, batch_x_peak=None, batch_y_peak=None, batch_x_idx=None, batch_y_idx=None):
        ...
        if self.args.inverse:
            pass
            # Why batch y is not inversed!? YEKTA Changed HERE
