...
class Exp_Informer(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        if flag == 'test':
            ...
            # Pred doesnt have train/test/val
            Data = Dataset_Pred
        else:
            # shuffle_flag is important!
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark, batch_x_peak, batch_y_peak, batch_x_idx, batch_y_idx) in enumerate(vali_loader):
            pred, true, peak_logits, base, delta = self._process_one_batch(
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            def asymmetric_mse(pred, true, y_peak, gamma=5.0):
                err = (pred - true).float()
                w_peak = 1.0 + gamma * y_peak.unsqueeze(-1)
                return (w_peak * err.pow(2)).mean()
            ...
            for i, (batch_x,batch_y,batch_x_mark,batch_y_mark, batch_x_peak, batch_y_peak, batch_x_idx, batch_y_idx) in enumerate(train_loader):
                ...
                cls_loss = F.binary_cross_entropy_with_logits(peak_logits.squeeze(-1), y_peak_gt, pos_weight=pos_w)
                loss_forecast = asymmetric_mse(pred, true, y_peak_gt, gamma=5.0)

                # Consider offset only if there is a peak
                amp_target = (true - base).clamp(min=0)
                mask = y_peak_gt.unsqueeze(-1)                    # [B, T, 1], 1 on peak steps
                amp_loss = F.l1_loss(delta * mask, amp_target * mask)
                
                alpha = 0.33
                beta = 0.33
                gamma = 0.33
                loss = (alpha*loss_forecast) + (gamma*cls_loss) + (beta * amp_loss)

                # # standard way
                # loss = criterion(base, true)
    ...
    def predict(self, setting, load=False):
        ...
        # batch_x -> 1x96x7 (7 features, 96 inp)
        # batch_y -> 1x48x7 (7 features, 48 decoder inp)
        # batch_x_mark -> 1x96x4 (time encoding for 4)
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark, batch_x_peak, batch_y_peak, batch_x_idx, batch_y_idx) in enumerate(pred_loader):
            ...
            peak_true = batch_y_idx[:, -self.args.pred_len:].float().unsqueeze(-1)
            gate = torch.sigmoid(peak_logits)
            # peak_binary = (gate > 0.5).float()
            peaks.append(gate.detach().cpu().numpy())
            peak_trues.append(peak_true.detach().cpu().numpy() )
        
        os.makedirs("res/"+setting, exist_ok=True)

        peaks = np.array(peaks)
        peaks = peaks.reshape(-1, peaks.shape[-2], peaks.shape[-1])
        peaks_2d = peaks.reshape(peaks.shape[-3], peaks.shape[-2]).astype(object)
        peaks_2d = pd.DataFrame(peaks_2d)
        peaks_2d.to_csv("res/"+setting+"/INF_peak.csv", index=False, header=False)

        peak_trues = np.array(peak_trues)
        peak_trues = peak_trues.reshape(-1, peak_trues.shape[-2], peak_trues.shape[-1])
        peaks_t_2d = peak_trues.reshape(peak_trues.shape[-3], peak_trues.shape[-2]).astype(object)
        peaks_t_2d = pd.DataFrame(peaks_t_2d)
        peaks_t_2d.to_csv("res/"+setting+"/INF_peak_true.csv", index=False, header=False)
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, batch_x_peak=None, batch_y_peak=None, batch_x_idx=None, batch_y_idx=None):
        ...
        if self.args.use_amp:
            ...
            if self.args.output_attention:
                ...
                outputs, logits, base, delta = self.model(batch_x,
                                     batch_x_mark,
                                     dec_inp,
                                     batch_y_mark,
        ...
        return outputs, batch_y, logits, base, delta
